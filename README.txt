...
0. FastQ file sorting 
    ...
    -Initially, the only folder will be 'input', which contains subfolders that contain individual sequencing files in fastQ format (fwd and rev separate).
    -Use the 'samples_to_csv.py' script to gather all filenames in the directories. The script will also parse the experimental conditions based on the file name.
...
4. Calculate fitness score
    ...
    -Run the 'calculate_fitness_score.py' script. Results are saved in 'scoring'.
